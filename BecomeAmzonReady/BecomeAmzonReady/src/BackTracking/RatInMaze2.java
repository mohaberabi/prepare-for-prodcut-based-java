...
public class RatInMaze2 {
    ...
    public static int[][] visited = new int[5][5]; // 2D array to keep track of visited cells
    public static ArrayList<String> result = new ArrayList<String>(); // ArrayList to store the path results
    ...
    public static void path(int[][] m, int x, int y, String dir, int n) {
        ...
        if (x > 0) {
            path(m, x - 1, y, dir + 'U', n); // recursively call the path function for the cell above
        }
        if (y > 0) {
            path(m, x, y - 1, dir + 'L', n); // recursively call the path function for the cell on the left
        }
        if (x < n - 1) {
            path(m, x + 1, y, dir + 'D', n); // recursively call the path function for the cell below
        }
        if (y < n - 1) {
            path(m, x, y + 1, dir + 'R', n); // recursively call the path function for the cell on the right
        }
    ...
    public static ArrayList<String> findPath(int[][] m, int n) {
        try {
            Arrays.fill(visited, 0); // initialize the visited array with 0
        ...
        path(m, 0, 0, "", n); // start finding paths from the top-left cell
